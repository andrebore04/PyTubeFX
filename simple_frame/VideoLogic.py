...
from pytubefix import YouTube, Stream, exceptions as pytube_exceptions
...
class VideoLogic():
    ...
    def check_download(self) -> None:
        ...
        if self.thread.is_alive():
            self.paired_frame.set_status(f'Downloading {self.download_percentage}%'
                                          f' [{next(self.char_generator)}]')
            self.paired_frame.after(300, self.check_download)
        else:
            logger.info('check_download()[Main Thread] Thread finished.\n')
            self.paired_frame.set_status(f'Downloading completed! Choose where to save the video.')
            self.paired_frame.after(0, self.download_complete)
    
    def download_complete(self) -> None:
        """
        Method that prepares the downloaded files,
        asks the user the save path of the video,
        and calls `VideoLogic.process_ffmpeg`
        """
        ...
        if not output_path: # User didn't choose a file and closed the dialog
            ...
            try:
                ...
                VideoLogic.process_ffmpeg(self.video_path,
                                           self.audio_path,
                                           output_path)
        ...
        self.paired_frame.set_status(new_status)
        shutil.rmtree(self.temp_dir)
        self.temp_dir = '' # This is now invalid

        # Re-enable widgets
        self.paired_frame.set_export_widgets(True)
        self.paired_frame.set_process_button(True)
        
        # Reset callbacks
        self.paired_frame.set_input_callback_functions(self.process_user_input,
                                                        self.process_user_input)
        self.paired_frame.set_export_callback_function(self.start_download)
    ...
    def process_user_input(self, _: Optional[Event] = None):
        ...
        """
        user_input = self.paired_frame.get_input_text()
        ...
        self.paired_frame.set_export_widgets(enabled=enable_export_widgets)
        self.paired_frame.set_status(new_status_message=status)
    ...
    def start_download(self):
        ...
        """
        selected_resolution = self.paired_frame.get_selected_resolution()
        selected_bitrate = self.paired_frame.get_selected_bitrate()
        ...
        if not video_itag and not audio_itag:
            ...
            self.paired_frame.set_status('No stream selected, nothing happened!')
            return
        ...
        self.char_generator = VideoLogic.animation_charset(0) # To display download animation
        ...
        # Disable widgets
        self.paired_frame.set_export_widgets(False)
        self.paired_frame.set_process_button(False)
        
        # Disable callbacks
        self.paired_frame.set_input_callback_functions()
        self.paired_frame.set_export_callback_function()

        # Start thread-checking loop after half a second
        self.paired_frame.after(500, self.check_download)
    ...
    def update_frame(self):
        ...
        self.paired_frame.set_resolution_list(
        ...
        self.paired_frame.set_bitrate_list( # Sort by audio bitrate
        ...
        self.paired_frame.set_video_title(self.video_object.title)
        ...
        self.paired_frame.set_video_thumbnail(thumbnail)